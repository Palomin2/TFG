...
/*
*Input: n=the size of the Nurbs vector-p-2, p polynomial degree, KnotVector, the CtrlPts ,
*        t the point of evalution and d highest derivate order to compute
...
/*
*Input: n=the size of the Nurbs vector-p-2, p polynomial degree, KnotVector, the CtrlPts ,t the point of evalution, 
*         d highest derivate order to compute, r1 and r2 specify the intervals of CtrlPoints
*Output: iMatrix where the i-th row is the vector of ctrl points
*/
iMatrix<std::vector<double>> CurveDerivsCpts(int n, int p,const std::vector<double> &KnotVector,const iMatrix<double> &CtrlPts,
                                             int d, int r1, int r2);
...
*/
std::vector<double> SurfacePoint(int n1, int p1,const std::vector<double> &KnotVector1, int n2, int p2,const std::vector<double> &KnotVector2,
                                 const std::vector<iMatrix<double>> &CtrlPts, double t1, double t2);

/*
*Input: n_i=the size of the Nurbs vector_i-p-2, p_i polynomial degree, KnotVector_i, the CtrlPts ,
*       t_i the point of evalution and d maximum order of the derivates
*Output: Evaluation of the derivates up to degree d (maximum is p or q and would overwrite it if d>p,q) in (t1,t2) where Output[k_1][k_2] is been
         derived k_i times respect to the ith variable 
*/
iMatrix<std::vector<double>> SurfaceDerivsAlg1(int n1, int p1,const std::vector<double> &KnotVector1, int n2, int p2,
        const std::vector<double> &KnotVector2,const std::vector<iMatrix<double>> &CtrlPts, double t1, double t2, int d);
...
/*
*Input: Vector of CtrlPts and their weights at each point in a separated vector
*Output: std::vector<iMatrix<double>> that keeps the format of the CtrlPts for Surface
         operations with and extra 4th dimension on each coord thats's occupied by the weight of such pt 
*/
...
/*
*Input: n=the size of the Nurbs vector-p-2, p polynomial degree, KnotVector, the CtrlPts weighted with theris weight in the last component ,
        t the point of evalution
...
/*
*Input: i span index, t eval point, k derivate degree, p polinomial degree and KnotVector, iMatrix of the derivates of the non-rational
*         basis funs and weights vector
*Output: vector of rational basis funcs
*/
iMatrix<double> RationalizeDersBasisFuns(int i, double t, int p, int k,  const std::vector<double> &KnotVector,const iMatrix<double> &ders ,
         const std::vector<double> &WeightVector);

/*
*Input: Aders: derivates of the CtrlPtsWeighted using CurvDersAlg1 evaluated at some t, wders: same as Aders but with the weighted vector,
*         maximum degree of the derivate
...
*Output: Evaluation of the surface at point (t1,t2)
*/
std::vector<double> SurfacePointRational(int n1, int p1,const std::vector<double> &KnotVector1, int n2, int p2,
        const std::vector<double> &KnotVector2,const std::vector<iMatrix<double>> &CtrlPtsWeighted, double t1, double t2);

/*
*Input: derivates of the CtrlPtsWeighted using CurvDersAlg1 evaluated at some t, wders: same as Aders but with the weighted vector,
*         maximum degree of the derivate
...
*Note: Applying insertion with this index using KnotVector.insert(KnotVector.begin()+index+1, r, e) will yield the KnotVector
*         we want (r is number of insertions)
*/
...
/*
*Input: npi size of KnotVectori-pi-2, pi degree of KnotVectori, KnotVectori for i=1,2, CtrlPts Weigthed, dir(true= knot inserted in Knotvector1,
         false= knot inserted in Knotvector2, uv knot we want to insert, index such that KnotVectori[k]<= u < KnotVectori[k+1], s initial
          multiplicity of u in KnotVectori, r number of insertions of u in Knotvectori
*Output: the KnotVectori will be updated and it will return new CtrlPtsW such that they generate the same surface.
*/
std::vector<iMatrix<double>> SurfaceKnotIns(int np1, int p1, std::vector<double> &KnotVector1, int np2, int p2, 
        std::vector<double> &KnotVector2,const std::vector<iMatrix<double>> &CtrlPtsW, bool dir,
                    double uv, int index, int r, int s);

/*
*Input: n=Knotvector.size()-2-p, p polynomial degree, KnotVector, CtrlPtsW, Insertions: vector of new elements to insert into KnotVector,
         r=Insertion.size()-1. 
*Output: Update of the Knotvector with the new inserions, CtrlPtsW that generate the same curve with the new KnotVector.
*/
iMatrix<double> RefineKnotVectCurve(int n, int p, std::vector<double> &KnotVector,const iMatrix<double> &CtrlPtsW,
        const std::vector<double> &Insertions, int r);

/*
*Input: npi size of KnotVectori-1, pi degree of KnotVectori, KnotVectori for i=1,2, CtrlPts Weigthed, dir(true= knot inserted in Knotvector1,
         false= knot inserted in Knotvector2, Insertions: vector of new elements to insert into KnotVector,  r=Insertion.size()-1.
*Output: the KnotVectori will be updated and it will return new CtrlPtsW such that they generate the same surface.
*/
std::vector<iMatrix<double>> RefineKnotVectSurface(int np1, int p1, std::vector<double> &KnotVector1, int np2, int p2,
                 std::vector<double> &KnotVector2,const std::vector<iMatrix<double>> &CtrlPtsW, bool dir,
                    const std::vector<double> Insertions, int r);
...
*        KnotVector, upper and lower limits of the integration, weights vector, nodes of the legendre polynomial,  CtrlPtsW the control
*        points weighted, the function f and the previoues evalPoint.
*Output: BasisFunsEvals and DerBasisFunsEvals each row will store the evaluation of the value or derivate of the basis fun alongside each
*        evaluation point, same for the JacobianEvals, InverseJacobianEvals and funcEvals.
*/
void D1_element_eval(int n, int i, int p, int nEvals, double lower_limit, double upper_limit,const std::vector<double> &KnotVector,
        const std::vector<double> &Weights, const Eigen::VectorXd &nodes,const iMatrix<double> &CtrlPtsW, std::function<double(double)> f,
        double &preEvalPoint,  double &cumLenght, iMatrix<double> &BasisFunsEvals, iMatrix<double> &DerBasisFunsEvals,std::vector<double>& JacobianEvals,
         std::vector<double>& InverseJacobianEvals, std::vector<double>& funcEvals, double Lenght=1.0);

/*
*Input: p polinomial degree, upper and lower limits of the integration, number of points of evaluations in the integral,
         evaluations of RationalDerivates, evaluation of the Inverse Jacobian and the legendre weights.
*Output: Matrix of all the permutations of evaluations where the elements i,j of the matrix correspond to
         the BasisFuns(init+i)*BasisFuns(init+j) where init=spanIndex()
*/                        
iMatrix<double> gauss_legendre_cuadrature_integral_bilinealForm(int p, double lower_limit, double upper_limit, int nEvals,
        const iMatrix<double> &DerBasisFunsEvals,const std::vector<double>& InverseJacobianEvals,const Eigen::VectorXd &weights);

/*
*Input: p polinomial degree, upper and lower limits of the integration, number of points of evaluations in the integral,
         evaluations of RationalBasisFuns, evaluations of the Jacobian, evaluations of the function and the legendre weights.
*Output: Vector of evaluations which its components are l(index) where init=spanIndex()
*/                        
std::vector<double> gauss_legendre_cuadrature_integral_linealForm(int p, double lower_limit, double upper_limit,
         int nEvals,const iMatrix<double> &BasisFunsEvals,const std::vector<double>& JacobianEvals,const std::vector<double>& funcEvals,
         const Eigen::VectorXd &weights);

/*
*Input: p polinomial degree, size=LineraForm.size()-1, SparseMatrix with the system already assambled, 
        same with the LinearForm, Modify0(1) true if we have dirichlet condition at the start(end) of the boundary,
...
*/  
void impose_Dirichlet_Condition(int p, int size, Eigen::SparseMatrix<double> &global, Eigen::VectorXd &LinearForm,
         bool Modify0, bool Modify1, double ValueAt0=0.0, double ValueAt1=0.0);
...
/*
*Input: p polinomial degree, size=LineraForm.size()-1, SparseMatrix with the system already assambled, same with the LinearForm,
         Modify0(1) true if we have dirichlet condition at the start(end) of the boundary,
*       vValueAt0(1) value set for the boundary condition, default is set at 0.
*       values alpha and beta come from the expresion alpha*u(x)+beta*u'(x)=ValueAtx.
*Output: Modification of the sparse matrix and the linear form to keep the system as symetric as posible.
*/  
void impose_Robin_Condition(int p, int size, Eigen::SparseMatrix<double> &global, Eigen::VectorXd &LinearForm, double alpha,
         double beta, bool Modify0, bool Modify1, double ValueAt0=0.0, double ValueAt1=0.0);
...
*/  
void writeNumericFunction(Eigen::VectorXd funEvals,const std::vector<double> &KnotVector,const std::vector<double> &WeightVector,
         int n, int p, int nEvals, double lowerLimit, double upperLimit,  std::string name,
         std::vector<double> Intervals, int nElements, double Lenght); 

/*
*Input: the function we evaluate, lower and upper limits of evaluation and number of steps of the evaluation, name of the 
        file where we write the evaluations, for the evaluation of the curve point, CtrlPtsW, KnotVector, n = KnotVector.size()-p-2,
         p polynomial degree.
*Output: writes the evaluation of the function in a txt
*/  
void writeAnalyticFunction(std::function<double(double)> f,std::function<double(double)> f_Ders,std::vector<double> Intervals, double nEvals,
        const iMatrix<double> &CtrlPtsW,const std::vector<double> &KnotVector, int n, int p, std::string name, double Lenght, double nElements);
...
*/
/*
*Input: the function we evaluate, lower and upper limits of evaluation and number of steps of the evaluation, name of hte file
         where we write the evaluations, for the evaluation of the curve point, CtrlPtsW, KnotVector, n = KnotVector.size()-p-2, p polynomial degree.
*Output: writes the evaluation of the function in a txt
*/ 
/* 
void writeFunctionDers(std::function<double(double)> f, double lowerLimit, double upperLimit, double nEvals,
        const iMatrix<double> &CtrlPtsW,const std::vector<double> &KnotVector,
...
*/ 
double IntegrateNormDer(double lowerLimit, double upperLimit, int nEvals,const iMatrix<double> &CtrlPtsW,
        const std::vector<double> &KnotVector, int n, int p);

/*
*Input: e fisical parameter, lowerLimit of integration, upperLimit of integration, number of subintervals for the quadrature,
         CtrlPtsW, Knotvector, n, p
...
*/ 
inline double Fisical_to_Parametric(double t2, double t1, double lowerLimit, double Lenght,
         int nEvals,const iMatrix<double> &CtrlPtsW,const std::vector<double> &KnotVector, int n, int p){
        return IntegrateNormDer(t2,t1,nEvals, CtrlPtsW, KnotVector, n, p)/Lenght;
}

/*
*Input: i span index, p polynomial degree, k maximum derivate order, KnotVector, Intervals, nodes and weights
         of the gauss-legendre cuadrature, vector of storage with spanIndex(memory has to be reserved before execution).
*Output: vector that each element is a vector that evaluates the basis functions in each of the corresponding gauss-legendre
         points of each interval, modification of the vector spanIndex.
*/ 
std::vector<std::vector<iMatrix<double>>> PreBasis_and_Ders(int p, int k, std::vector<double> KnotVector, std::vector<double> Intervals,
        Eigen::VectorXd nodes, std::vector<int>& spanIndex);

/*
*Input: span index,nEvals number of points of the gauss-legendre cuadrature alongside the corresponding variable, nodes,
        vector of the Gauss-Legendre cuadrature, lower_limit, upper_limit of the interval, p polynomial degree, k maximum derivate order, KnotVector,
        Basis_and_DersY vector of the precomputed Basis and its corresponding derivates, nodes and weights of the gauss-legendre cuadrature 
        and Weights vector of the corresponding weights alongside the corresponding variables.
*Output: vector that each element is thee evaluations of the basis functions and its derivates up to k-degree in each of the corresponding 
        gauss-legendre points of the interval.
*/ 
std::vector<iMatrix<double>> RationalizeDersBasisFunsVec(int span, int nEvals, Eigen::VectorXd nodes, double lower_limit, double upper_limit,
        int p, int k, int n, std::vector<double> KnotVector,std::vector<iMatrix<double>> Basis_and_DersY,std::vector<double> Weights);
...
*Output: unordered map such that given a span index returns its corresponding interval.
*/ 
iMatrix<iMatrix<double>> RationalizeDersBasisFuns2D(const iMatrix<double> &DersBasisX,const iMatrix<double> &DersBasisY,
        const iMatrix<double> &activeWeights, unsigned int pX, unsigned int pY);
