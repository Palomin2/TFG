...
refineCount = 2; % como maximo haremos 8 ref en 2D
max_level = 2;
...
Pw = zeros(noPtsX, noPtsY, 4);
for i = 1:noPtsX
    for j = 1:noPtsY
        idx = (j-1)*noPtsX + i;
        w = weights(idx);
        Pw(i,j,1) = controlPts(idx,1)*w;
        Pw(i,j,2) = controlPts(idx,2)*w;
        Pw(i,j,3) = U(idx)*w;  % si U es el desplazamiento/solución
        Pw(i,j,4) = w;
    end
end
