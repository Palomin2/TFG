...
s = zeros(size(u));
for j = 1:numel(u) % numel = number of array elements. En RefineKnot, llamamos a esta funcion con numel(u)=1
  if (u(j)==U(n+2)), s(j)=n; continue, end
  s(j) = find(u(j) >= U,1,'last')-1; %encontrar el indice k del vector de nudos U tal que u >= U(k)
% Realmente lo que hace es buscar el primer indice desde el final de un
% elemento en U tal que se cumpla la condicion.
end
